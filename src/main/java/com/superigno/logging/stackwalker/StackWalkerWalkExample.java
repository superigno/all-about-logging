...
public class StackWalkerWalkExample {
	...
	private static List<String> fetchLines(Stream<StackWalker.StackFrame> stream) {
		...
		List<String> lines = stream.filter(stackFrame -> stackFrame.getMethodName().startsWith("method")).map(stackFrame -> stackFrame.getMethodName() + ":" + stackFrame.getLineNumber()).collect(Collectors.toList());
		return lines;
	}
